...
const ModifierBateau = ({ idBateau, nom, capaciteBateau }) => {
    const [nomBateau, setNomBateau] = useState(nom || '');
    const [capacite, setCapacite] = useState(capaciteBateau || 0);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (response.status === 200) {
                notification("Bateau modifier avec succès 🎉", "success");
    ...
    return (
        <div id="formAjoutBateau">
            <button onClick={openModal} className="btn btn-primary">Modifier</button>
